...
def initialize_pinecone():
    ...
    if vector_store is None:
        ...
        """if index_name not in pc.list_indexes().names():
            pc.create_index(
                name=index_name, 
                dimension=768,  # Use the correct dimension for your embeddings
                metric='cosine'  # Use an appropriate metric like 'cosine', 'euclidean', or 'dotproduct'
            )"""
        index = pc.Index(index_name)

        # Create the LangChain Pinecone vector store
        embeddings = download_hugging_face_embeddings()
        vector_store = Pinecone(index, embeddings.embed_query, "text")  # or "text" depending on your setup
...
def chat():
    ...
    #question = result.get("query", "No question found")
